...
import { buildSparseBasis } from './parametric';
import { localInterrogationGradientStep } from './mld_local';
import type { IterationSnapshot, ParametricSupportConfig, SampledDataset, SparseBasisMask, TrainOptions } from './parametric';
...
class XorShift32 {
  private x: number;
  constructor(seed: number) { this.x = seed | 0 || 246813579; }
...
function trainImageClassifier(trainRecords: ImageFeatureRecord[], valRecords: ImageFeatureRecord[], config: ParametricSupportConfig, opts: TrainOptions): ImageParametricTrainingResult {
  const basis = buildSparseBasis(config);
  const train = buildAugmentedDataset(trainRecords, config.seed ^ 0x55aa, opts.trainCount, 0.05);
  ...
  for (let iter = 0; iter < opts.iterations; iter++) {
    ...
    const invMc = 1 / Math.max(1, opts.mcSamples);
    ...
    const scaleObjective = (candidateLogScales: number[]): number => {
      ...
      const penalty = candidateLogScales.map(Math.exp).reduce((s, x) => s + x * x, 0) / candidateLogScales.length;
      return acc / Math.max(1, zs.length) - opts.scalePenalty * penalty;
    };
...
export function defaultImageParametricDemo(outDir = 'runs/image-parametric-demo', imagePaths?: string[]): ImageParametricTrainingResult {
  ...
  const opts: TrainOptions = {
    ...
    logScaleMax: Math.log(1.5),
    snapshotEvery: 10
  };
  const result = trainImageClassifier(trainRecords, valRecords, config, opts);
