...
if not exist "glc\node_modules" (
    ...
    if exist "!CANDIDATE_SHARED_GLC_NODE_MODULES!" (
        echo [INFO] Linking glc dependencies from main checkout.
        echo [INFO] !CANDIDATE_SHARED_GLC_NODE_MODULES!
        mklink /J "glc\node_modules" "!CANDIDATE_SHARED_GLC_NODE_MODULES!" >nul
        if errorlevel 1 (
            echo [ERROR] Failed to link glc dependencies from main checkout.
            exit /b 1
        )
    )
)
