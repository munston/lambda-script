...
helpText =
  unlines
    ...
    , ""
    , "sandbox dispatch:"
    , "  onepush -> onepush Hat script"
    , "  land    -> land Hat script"
    , "  Hat is executed with `cabal run src/hat -- <script>.hat ...`."
    , "  Sandbox does not call .bat files."
    , ""
    , "sandbox.json fields:"
    , "  tool_root: trusted tooling root containing Hat and access scripts"
    , "  button: target button stem, e.g. lambda-script-edd"
    , "  hat_root: optional Hat package root; defaults to tool_root/hat"
    , "  onepush_hat: optional onepush script; defaults to tool_root/onepush-<button>.hat"
    , "  land_hat: optional land script; defaults to tool_root/land-anything.hat"
    ]
